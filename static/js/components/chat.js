...
function getBotResponse1(text){
    botResponse1 = `<img class="botAvatar" src="./static/img/sara_avatar.png"/><span class="botMsg">${text}</span><div class="clearfix"></div>`;
    return botResponse1;
}
        ...
        if (response.length < 1) {
            ...
            for (let i = 0; i < response.length; i += 1) {
                ...
                if (Object.hasOwnProperty.call(response[i], "custom")) {
                    ...
                    // check if the custom payload type is "video_attachment"
                    if (payload === "video_attachment") {
                        renderVideoAttachment(response[i]);
                        return;
                    }
                    ...
                    if (payload === "dropDown") {
                        ...
                        renderDropDwon(dropDownData, DDtitle);
                        return;
                    }
					
					// check if the custom payload type is "form"
                    if (payload === "form") {
                        const form_data = response[i].custom.data;
                        renderForm(form_data);
                        return;
                    }
